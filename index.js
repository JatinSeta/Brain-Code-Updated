...
window.submitLogin = function (event) {
  ...
  const email = form.email.value;
  const password = form.password.value;
  const rememberMe = form.rememberMe?.checked; // Add checkbox with name 'rememberMe'

  const persistenceType = rememberMe
    ? firebase.auth.Auth.Persistence.LOCAL   // Keeps user logged in across sessions
    : firebase.auth.Auth.Persistence.SESSION; // Only until tab/window is closed
...
document.addEventListener("DOMContentLoaded", () => {
  ...
  document.addEventListener('click', function (event) {
    ...
    function closeDropdownAndGoToAdmin() {
    document.getElementById('user-dropdown').classList.add('hidden'); // close dropdown
    window.location.href = './Admin.html'; // navigate to admin
  }